...
DeleteFromTb() {
    ...
    primary_key_column=""
    current_column=""
    pk_index=0  # FIXED: Track the column position (1-based for awk)
    col_counter=0
    ...
    echo "Primary Key Column: $primary_key_column (Column $pk_index)"
...
EOF
    ...
    # FIXED: Check if a record with the given primary key value exists (any column position)
    if ! awk -F'|' -v pk="$pk_value" -v idx="$pk_index" '
        $idx == pk { found=1; exit }
        END { exit !found }
    ' "tables/$TableSelectDel"; then
    ...
    # FIXED: Perform the deletion using awk to match exact column position
    awk -F'|' -v pk="$pk_value" -v idx="$pk_index" '
        BEGIN { OFS="|" }
        $idx != pk { print $0 }
    ' "tables/$TableSelectDel" > temp_table && mv temp_table "tables/$TableSelectDel"
