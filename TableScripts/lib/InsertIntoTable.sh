function InsertTb {
    ...
    local PrimaryKeyIndex=0  # FIXED: Store PK column position
        ...
        case "$line" in
            ...
            "Primary Key":*)
                ...
                if [[ "$pk_value" == "y" || "$pk_value" == "yes" ]]; then
                    ...
                    PrimaryKeyIndex=${#Columns[@]}  # FIXED: Store 1-based index
                fi
                ;;
        esac
    ...
    do
        ...
        do
            ...
            # FIXED: Rule 3: Prevent duplicate primary key values (check exact column position)
            if [[ "$col" == "$PrimaryKey" && -n "$value" ]]; then
                # Use awk to check if PK value exists in the correct column
                if [[ -s "$table_file" ]]; then
                    pk_exists=$(awk -F'|' -v pk_val="$value" -v pk_idx="$PrimaryKeyIndex" '
